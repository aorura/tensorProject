...
xy=np.loadtxt('data2/zoo.csv', delimiter=',', dtype=np.float32)
...
# 예) [[1,],[3]] -> None, 1인 shape을 원핫 인코딩 수행 후 [[[0100000], [[0001000]]]이 됨. rank :2 -> rang:3
#[None, 7]가 우리가 원하는 형태임.  reshape 함수 사용하면 됨.
#예) tf.reshape(yonehot, [-1, 7]로 주면  shape=(None, 7)이 됨
#최종 결과 [[0100000], [0001000]]
...
prediction=tf.argmax(hf,axis=1) # tf에서는 1 이  행단위
correct_prediction=tf.equal(prediction, tf.argmax(yonehot,axis=1))
