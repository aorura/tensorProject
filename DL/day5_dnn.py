# 털과 날개가 있는지 없는지에 따라, 포유류/조류인지 분류하는 팁러닝 모델 설계
#신경망의 레이어를 여러개로 구성
...
# 두번째 가중치의 차원은 [첫번째 히든 레이어의 뉴런 갯수, 분류개수]->[10,3]으로 한다.
...
# 신경망의 히든 레이어에 가중치  w1과 b1을 적용
L1=tf.add(tf.matmul(x,w1), b1)
L1=tf.nn.relu(L1)   # relu : 음수가 들어가면 0, 양수가 들어가면 양수
